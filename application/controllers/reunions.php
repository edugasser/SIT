...
class reunions extends CI_Controller {
	...
	public function mio(){
			$sql4 = "SELECT *,DATE_FORMAT(data,'%d-%m-%Y') as data FROM reunion";
			$data['data'] = $this->mi_model->get_sql($sql4);	

			$this->load->view('reunions/gestion_view2', $data);
	 
	}
	public function gestion($success=null){
		if ($this->tank_auth->is_logged_in()) {		
			$data['contenido'] =  "reunions/gestion_view";
			$this->load->view('page_view', $data);
		}else{
			redirect('auth/');
		}
